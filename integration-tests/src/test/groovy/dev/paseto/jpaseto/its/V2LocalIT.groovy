...
class V2LocalIT {
    ...
    void officialVectorsV2LocalEncodeTest(String token, SecretKey secretKey, Map<String, Object> claims, Map<String, Object> footer, String name) {
        ...
        PasetoBuilder pasetoBuilder = Pasetos.V2.LOCAL.builder().setSharedSecret(secretKey)

        PasetoV2LocalTokenBuilder pasetoV2LocalTokenBuilder = Pasetos.V2.LOCAL.tokenBuilder()

        claims.forEach { key, value ->
            pasetoV2LocalTokenBuilder.claim(key, value)
        }
        if (footer) {
            footer.forEach { key, value ->
                pasetoV2LocalTokenBuilder.footerClaim(key, value)
            }
        }

        String result = pasetoBuilder.compact(pasetoV2LocalTokenBuilder)
    ...
    void encodeAndParseWithGeneratedKey() {
        ...
        def tokenString = Pasetos.V2.LOCAL.builder()
            .setSharedSecret(key)
            .compact(Pasetos.V2.LOCAL.tokenBuilder().claim("test-name", name))
